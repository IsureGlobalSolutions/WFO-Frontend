...
import { addEditFte, getFteData, OrganizationFteById } from "../../redux/features/auth/authSlice";
...
function OrganizationFTEModal({ onBack, setActive ,selectedId}) {
  ...
  useEffect(() => 
    ...
    if (selectedId) {
      setIsLoader(true);
      dispatch(OrganizationFteById(selectedId)).then((res) => {
        if (res?.payload) {
          setFormValues({
            fte: {
              ...res.payload,
  ...
  const handleSubmit = async (e) => {
    ...
    // if (isValid) {
      try {
        ...
        await dispatch(addEditFte(transformedData.fte))
        .then((res) => {
            ...
            {formValues.fte.breakCustomFields.map((field, index) => (
              <div key={index} className="d-flex align-items-center mb-2">
                ...
                <input
                  type="text"
                  className="form-control me-2 bg-light"
                  value={field.fieldValue.replace(".", ":")} // Display as HH:mm
                  readOnly
                  style={{ maxWidth: "120px" }}
                />
                <button
...
<hr className="m-0 mb-2 pt-4" />
          {/* Gazetted Holidays */}
          <div className="mb-3">
        <div className="d-flex align-items-center mb-2">
          <h5 className=" fw-bolder me-2">Gazetted Holidays</h5>
        </div>
...
export default OrganizationFTEModal;
