...
function TeamFTEModal({ onBack ,  setActive, selectedId }) {
  ...
  useEffect(() => {
    setIsLoader(true);
    if (selectedId) {
      dispatch(getTeamFteById(selectedId)).then((res) => {
        if (res.payload) {
          ...
          const project = Teamname.find((item) => item.id === res.payload.teamId);
          if (project) {
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      await dispatch(addEditTeamtFTE(transformedData.fte))
        .then((res) => {
          if(res?.payload?.isSuccess){
            dispatch(getTeamFteData());
