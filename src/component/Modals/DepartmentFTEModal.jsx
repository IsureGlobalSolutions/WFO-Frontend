...
function DepartmentFTEModal({ onBack ,  setActive, selectedId }) {
  ...
  useEffect(() => {
    setIsLoader(true)
    if (selectedId) {
      dispatch(getDepartmentFteById(selectedId)).then((res) => {
        if (res.payload) {
          setFormValues({
            fte: {
              ...res.payload,
          ...
          const department = Departmentname.find((item) => item.id === res.payload.departmentId);
          if (department) {
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      await dispatch(addEditDeparmentFTE(transformedData.fte))
        .then((res) => {
...
export default DepartmentFTEModal;