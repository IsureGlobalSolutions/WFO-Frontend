...
import { deleteRole, userRole } from "../../redux/features/auth/authSlice";
...
import ViewPermission from "../Modals/ViewPermission";
import {ReactComponent as ViewPermissionicon} from '../../assets/img/dashboard/viewPermissionIcon.svg'
...
const RoleList = ({ selectedModule }) => {
  const dispatch = useDispatch();
  const { permissions } = useContext(AuthContext);
  

  const ftePermission = permissions?.userPermissions?.find(
    perm => perm.permissionName === "Roles Detail"
  ...
  // Selectors
  const roles = useSelector((state) => state.auth.userRolename);
  ...
  const filteredData = React.useMemo(() => {
    if (!selectedColumn || !searchValue.trim()) return roles || [];

    const columnKey = columnMap[selectedColumn];
    if (!columnKey) return roles || [];

    return (roles || []).filter(item => {
      // Handle nested permissions differently
      if (columnKey === 'userPermissions') {
        return item.userPermissions.some(perm => 
          perm.permissionName.toLowerCase().includes(searchValue.toLowerCase()) ||
          perm.subPermissions.some(sub => 
            sub.subPermissionName.toLowerCase().includes(searchValue.toLowerCase())
          )
        );
      }
  ...
  }, [roles, selectedColumn, searchValue]);
  ...
  const confirmDelete = async () => {
    try {
      setIsLoading(true);
      const response = await dispatch(deleteRole(deleteId));
      if (response.payload?.isSuccess) {
        setShowDeleteModal(false);
        await fetchRoles(); // Refresh the list
        toast.success("Role deleted successfully");
      } else {
        toast.error(response.payload?.message || "Failed to delete role");
      }
    } catch (error) {
  ...
  const openModalHandler=(data)=>{
    console.log("🚀 ~ openModalHandler ~ data:", data)
                  ...
                  paginatedData.map((item, id) => (
                    <tr key={item.employeeRoleId || id}>
                      ...
                      <td style={{
                        ...
                        {item.employeeRoleName}
                      </td>
                      <td style={{ borderBottom: '1px solid #dee2e6' }}>
                        {item.description}
                      </td>
                      <td style={{ borderBottom: '1px solid #dee2e6', cursor:'pointer' }}>
                      <div className="d-flex align-items-center justify-content-center gap-1"
                      onClick={()=>openModalHandler(item.userPermissions)}
                      >

                             
<ViewPermissionicon/>
<p className="m-0">View Permission</p>
                      </div>
                      </td>
                      <td style={{ borderBottom: "1px solid #dee2e6", width: "150px", textAlign: "end" }}>
                        ...
                        >
                          <button
                            ...
                            onClick={() => handleEdit(item.employeeRoleId)}
                            aria-label={`Edit role ${item.employeeRoleName}`}
                          >
                            <EditIcon />
                          </button>
                          <button
                            ...
                            onClick={() => handleDelete(item.employeeRoleId)}
                            aria-label={`Delete role ${item.employeeRoleName}`}
                            disabled={isLoading}
                          >
                            {isLoading && deleteId === item.employeeRoleId ? (
                  ...
                  <tr>
                    <td colSpan={arrData.length} style={{ borderBottom: '1px solid #dee2e6' }}>
                      {searchValue ? "No matching roles found" : "No roles available"}
                    </td>
                  </tr>
                )}
              </tbody>
       ...
       { openPermissionModal  && 
<ViewPermission
active={openPermissionModal}
setActive={(e)=>setopenPermissionModal(e)}
   ...
   <PermissionsModal
      ...
      <Modal show={viewModal} onHide={handleCloseViewModal}>
        <Modal.Header closeButton>
          <Modal.Title style={{ fontSize: "25px", fontWeight: "600" }}>Role Details</Modal.Title>
        </Modal.Header>
        <Modal.Body>
          {selectedItem ? (
            <div>
              <div className="d-flex mb-3">
                <div className="col-lg-4 col-md-4" style={{ fontSize: "15px", fontWeight: "bold" }}>Role Name</div>
                <div className="ms-4">{selectedItem.employeeRoleName}</div>
              </div>
              <div className="d-flex mb-3">
                <div className="col-lg-4 col-md-4" style={{ fontSize: "15px", fontWeight: "bold" }}>Description</div>
                <div className="ms-4">{selectedItem.description}</div>
              </div>
              <div className="mb-3">
                <div style={{ fontSize: "15px", fontWeight: "bold" }}>Permissions</div>
      ...
      {isLoading && viewMode === 'list' ? (
        ...
        <AddRole onBack={handleBackToList} onSuccess={fetchRoles} />
      ) : viewMode === 'edit' ? (
        <AddRoleModal roleId={editRoleId} onBack={handleBackToList} onSuccess={fetchRoles} />
      ) : null}
