...
import SubDepartmentEdit from "../../HRAdminEditAPIs/SubDepartmentEdit";
...
const Subdepartmentlist = ({ isSidebarExpanded }) => {
  const arrData = [
  ...
  const ftePermission = permissions?.userPermissions?.find(
    perm => perm.permissionName === "Sub-Department"
  ...
  const SubDepartmentList = useSelector((state) => state.auth.SubDepartmentList);
    const [loading, setloading] = useState(false)
  
  useEffect(() => {
    dispatch(getSubDepartment()).then(()=>{
      setloading(false)
    });
  }, []);
useEffect(() => {
if(SubDepartmentList?.length>0){
  setdatalist(SubDepartmentList)
}
  }, [SubDepartmentList]);
...
const endIndex = (filteredData?.length || 0) === 0
  ? 0
...
const paginatedData = filteredData?.length ? filteredData.slice(startIndex, endIndex) : [];
  ...
  const fetchmethod = async (e) => {
    try {
      dispatch(getSubDepartment()).then((response) => {
  ...
  const fetchdataid = async (id) => {
    try {
     const response = await dispatch(getJobSubDepartmentiddata(id));
...
const [deleteid, setdeleteid] = useState(null)
  const conformDelete = async () => { 
    try {
      const response = await dispatch(deletesubdepartment(deleteid));
