...
import EmployTypeEdit from "../../HRAdminEditAPIs/EmployTypeEdit";
...
const Employeelist = () => {
 ...
 const Employeenames = useSelector((state) => state.auth.employeeData);
  useEffect(() => {
  ...
  const fetchmethod = async (e) => {
    try {
      dispatch(getEmployetypeData()).then((response) => {
  ...
  const fetchdataid = async (id) => {
    try {
     const response = await dispatch(getEmployeeiddata(id));
...
const [deleteid, setdeleteid] = useState(null)
  const conformDelete = async () => { 
    try {
      const response = await dispatch(deleteEmployee(deleteid));
