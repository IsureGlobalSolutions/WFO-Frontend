...
import TeamEdit from "../../HRAdminEditAPIs/TeamEdit";
...
const Teamlist = ({ isSidebarExpanded }) => {
  const arrData = [
  ...
  const TeamNames = useSelector((state) => state.auth.Teamname);
  ...
  const fetchmethod = async (e) => {
    try {
      dispatch(getTeam()).then((response) => {
  ...
  const fetchdataid = async (id) => {
    try {
     const response = await dispatch(getJobTeamiddata(id));
...
const [deleteid, setdeleteid] = useState(null)
  const conformDelete = async () => { 
    try {
      const response = await dispatch(deleteTeam(deleteid));
