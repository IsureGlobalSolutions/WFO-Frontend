...
import ProjectEdit from "../../HRAdminEditAPIs/ProjectEdit";
...
const Projectlist = ({ isSidebarExpanded }) => {
  const arrData = [
  ...
  const projectList=useSelector((state)=>state.auth.GetProjectList)
  useEffect(() => {
    setloading(true);
    dispatch(getProject()).then(()=>{
    setloading(false)
    })
  }, []);
  useEffect(() => {
if(projectList?.length>0){
  ...
  const fetchmethod = async (e) => {
    try {
      setloading(true);
      dispatch(getProject()).then((response) => {
  ...
  const fetchdataid = async (id) => {
    try {
     const response = await dispatch(getJobProjectiddata(id));
...
const [deleteid, setdeleteid] = useState(null)
  const conformDelete = async () => { 
    try {
      const response = await dispatch(deleteProjectdata(deleteid));
  ...
  <LatestSekeltonLoader/>
    ...
    : 'Showing 0-0 of 0'} 
        ...
        >
          ...
          {totalPages > 0 && [...Array(Math.max(0, Math.floor(totalPages)))].map((_, index) => (
              <li
                key={index}
