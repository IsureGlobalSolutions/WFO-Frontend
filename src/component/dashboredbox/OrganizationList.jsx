...
import OrganizationFTEModal from "../Modals/OrganizationFTEModal";
...
function OrganizationList({ getfterender }) {
  const dispatch = useDispatch();
  const { permissions } = useContext(AuthContext);
  
  // Check permissions
  const ftePermission = permissions?.userPermissions?.find(
    perm => perm.permissionName === "Organization FTE"
  ...
  const organizationdata = useSelector((state) => state.auth.organizationdata);
  console.log("🚀 ~ OrganizationList ~ organizationdata:", organizationdata)
  ...
  useEffect(() => {
    if (!selectedColumn || searchValue.trim() === '') {
      ...
      const filtered = organizationdata.filter((item) => {
  ...
  }, [selectedColumn, searchValue, organizationdata]);
  ...
  const endIndex = organizationdata?.length === 0
    ? 0
    : Math.min(startIndex + itemsPerPage, organizationdata?.length);
  ...
  const fetchmethod = async () => {
    try {
      dispatch(getFteData()).then((response) => {
  ...
  const conformDelete = async () => { 
    try {
      const response = await dispatch(deleteFteData(deleteid));
      ...
      setPopupMessage("Error occurring FTE Not Deleted")   
    }
    setdeleteid(null);
  };
        ...
        activeAddModal ? (
          <div style={{backgroundColor:"#FFFFFF" ,border:"1px solid #E0E3E9" , marginTop:"1px"}}>
            <div className="d-flex justify-content-between align-items-center ms-2 p-4 ps-3 pb-0 mb-1">
              ...
              >
                Organization FTE List
              </p>
              <div className="d-flex ali">
                <button 
                  type="button" 
                  className="save-button gap-2 d-flex align-item-center ps-4 pe-4 w-50" 
