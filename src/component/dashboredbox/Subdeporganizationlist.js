...
import { addEditFte, getSubdepFteData, deletesubdepData } from '../../redux/features/auth/authSlice';
import { MdErrorOutline } from "react-icons/md";
import {Button , Modal } from "react-bootstrap";
import { MdDeleteForever } from "react-icons/md";
import { FaRegEdit } from 'react-icons/fa';
import SubDepartmentFTEModal from '../Modals/SubDepartmentFTEModal';
...
function Subdeporganizationlist({getfterender}) {
  const dispatch = useDispatch();
  const { permissions } = useContext(AuthContext);
  
  const ftePermission = permissions?.userPermissions?.find(
    perm => perm.permissionName === "Sub-Department FTE"
  ...
  const SubdepartmentData = useSelector((state) => state.auth.SubdepartmentData );
 ...
 const [activeAddModal, setactiveAddModal] = useState(true);
   ...
   useEffect(() => {
     if (!selectedColumn || searchValue.trim() === '') {
       setFilteredData(SubdepartmentData);
     } else {
       const lowerSearch = searchValue.toLowerCase();
      
       const dataProperty = columnMap[selectedColumn];
      
       const filtered = SubdepartmentData.filter((item) => {
   ...
   }, [selectedColumn, searchValue, SubdepartmentData]);
   ...
   const endIndex = SubdepartmentData?.length === 0
     ? 0
     : Math.min(startIndex + itemsPerPage, SubdepartmentData?.length); // not paginatedData.length
...
const conformDelete = async () => { 
  try {
    const response =  await dispatch(deletesubdepData(deleteid));
...
if (!canViewFTE) {
  ...
  return (
    ...
    {activeEdit ? (<SubDepartmentFTEModal
active={activeEdit} 
onBack={() => setactiveEdit(false)}
selectedId={deleteid}
/>) : ( 
   activeAddModal ? (
    <div className='' style={{backgroundColor:"#FFFFFF" , border:"1px solid #E0E3E9" , marginTop:"1px"}}>     
...
>Sub-Department FTE List</p>
<div className=" d-flex">
<button type="button" class="blue-button"   onClick={() =>{
   if (!canAddFTE) {
    setShowPermissionModal(true);
    return;
  }
...
>
  {arrData?.filter(item => item !== "Action" && item !=="S.No") // Exclude "Action" from dropdown
       .map((item, idx) => (
      <div
        key={idx}
...
export default Subdeporganizationlist