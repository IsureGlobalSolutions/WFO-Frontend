...
import { addEditFte, getTeamFteData, deleteTeamFteData } from '../../redux/features/auth/authSlice';
import { MdDeleteForever } from "react-icons/md";
import { MdErrorOutline } from "react-icons/md";
import {Button , Modal } from "react-bootstrap";
import Team from './Team';
import TeamFTEModal from '../Modals/TeamFTEModal';
...
function Teamorganizationlist({getfterender}) {
  const dispatch = useDispatch();
  const { permissions } = useContext(AuthContext);
  
  const ftePermission = permissions?.userPermissions?.find(
    perm => perm.permissionName === "Team FTE"
  ...
  const TeamData = useSelector((state) => state.auth.TeamData );
  ...
  const [activeAddModal, setactiveAddModal] = useState(true);
      ...
      }, [selectedColumn, searchValue, TeamData]);
      ...
      const endIndex = TeamData?.length === 0
        ? 0
        : Math.min(startIndex + itemsPerPage, TeamData?.length); // not paginatedData.length
...
const [deleteid, setdeleteid] = useState(null)
  ...
  //   setPopupMessage("Are you want to Delete FTE data")
  //   setdeleteid(id);
  // }

  const conformDelete = async () => { 
    try {
      const response =  await dispatch(deleteTeamFteData(deleteid));
...
<PermissionsModal
    ...
    {activeEdit ? (
      ...
      <div style={{backgroundColor:"#FFFFFF" , border:"1px solid #E0E3E9", borderTop:"none" , marginTop:"2px"}}  >
...
export default Teamorganizationlist